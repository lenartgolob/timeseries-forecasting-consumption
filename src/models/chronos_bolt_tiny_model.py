"""
Chronos-bolt-tiny model from Amazon - a pretrained time series foundation model.
Zero-shot forecasting with transformer architecture.
"""
...
class Chronos_bolt_tinyModel(BaseModel):
    """
    Chronos-bolt-tiny forecasting model with zero-shot capabilities.
    
    cfg keys (all optional — default table shown):
    -------------------------------------------------------------
    model_name          | "amazon/chronos-bolt-tiny" | HuggingFace model ID
    ...
    def _setup_pipeline(self):
        ...
        print(f"   Loading Chronos model on {self._device} with {self._torch_dtype}")
    ...
    def save(self, path: Path) -> None:
        ...
        with open(Path(path) / "chronos_cfg.json", "w") as fp:
