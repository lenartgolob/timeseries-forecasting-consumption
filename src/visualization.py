...
def plot_performance_timeline(metrics_df, fig_dir):
    ...
    # Filter MAPE for plotting (remove inf/extreme values)
    mape_filtered = metrics_df['MAPE'].replace([np.inf, -np.inf], np.nan)
    mape_filtered = mape_filtered.clip(upper=1000)  # Cap at 1000%
    ax.plot(walks, mape_filtered, '^-', label='MAPE (%)', linewidth=2, markersize=4)
...
def plot_error_distribution(metrics_df, fig_dir):
    ...
    # MAE distribution
    axes[0].hist(metrics_df['MAE'], bins=20, alpha=0.7, color='steelblue', edgecolor='black')
    axes[0].axvline(metrics_df['MAE'].mean(), color='red', linestyle='--', linewidth=2,
            label=f'Mean: {metrics_df["MAE"].mean():.3f}')
    axes[0].set_xlabel('MAE (kWh)', fontsize=11)
    axes[0].set_ylabel('Frequency', fontsize=11)
    axes[0].set_title('MAE Distribution', fontsize=12, fontweight='bold')
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)    
    # RMSE distribution  
    axes[1].hist(metrics_df['RMSE'], bins=20, alpha=0.7, color='green', edgecolor='black')
    axes[1].axvline(metrics_df['RMSE'].mean(), color='red', linestyle='--', linewidth=2,
            label=f'Mean: {metrics_df["RMSE"].mean():.3f}')
    axes[1].set_xlabel('RMSE (kWh)', fontsize=11)
    ...
    mape_values = metrics_df['MAPE'].replace([np.inf, -np.inf], np.nan).dropna()
    mape_values = mape_values[mape_values <= 1000]  # Cap extreme values at 1000%    
    
    axes[2].hist(mape_values, bins=20, alpha=0.7, color='orange', edgecolor='black')
    axes[2].axvline(mape_values.mean(), color='red', linestyle='--', linewidth=2,
            label=f'Mean: {mape_values.mean():.2f}%')
    ...
    # nRMSE distribution
    axes[3].hist(metrics_df['nRMSE'], bins=20, alpha=0.7, color='purple', edgecolor='black')
    axes[3].axvline(metrics_df['nRMSE'].mean(), color='red', linestyle='--', linewidth=2,
            label=f'Mean: {metrics_df["nRMSE"].mean():.3f}')
    axes[3].set_xlabel('nRMSE', fontsize=11)
    axes[3].set_ylabel('Frequency', fontsize=11)
    axes[3].set_title('nRMSE Distribution', fontsize=12, fontweight='bold')
    axes[3].legend()
...
def plot_consumption_analysis(metrics_df, fig_dir):
    ...
    # Panel 1: Scatter plot - Consumption vs Error
    scatter = ax1.scatter(metrics_df['avg_consumption'], metrics_df['MAE'],
            c=range(len(metrics_df)), cmap='viridis', alpha=0.7, s=50)
    ...
    correlation = np.corrcoef(metrics_df['avg_consumption'], metrics_df['MAE'])[0, 1]
    ...
    line2 = ax2_twin.plot(metrics_df['walk'], metrics_df['MAE'],
